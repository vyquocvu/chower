...
function SideBar(props) {
  const { webSites, onSwitch } = props;
  return (
    <div className="bg-indigo-900 text-purple-300 flex-none w-16 p-2">
      {
        webSites.map((site, i) => <ItemSite switchSite={onSwitch(i)} site={site} index={i} key={i} />)
      }
      <div className="cursor-pointer" onClick={onSwitch(webSites.length)}>
